...
> ⚠️ **SUPERSEDED IN PART — read this banner first.**
> This is the original calendar+focus *brief* (background / rationale only). For
> **PR4b** the authoritative specs are **`CALENDAR_CELL_SPEC.md`** (visual),
> **`DUAL_DATE_SPEC.md`** (domain), and **`HIJRI_MONTH_ABBREVIATIONS.md`** (boundary
> labels). Where this file disagrees, **they win.** Specifically:
> - **Numeral position:** §A says "top-right / bottom-left" — **SUPERSEDED.** Use
>   `CALENDAR_CELL_SPEC`: both numerals are **top-center / bottom-center**;
>   `primaryHijri` (= user-set `isHijriMode`) chooses which is large. Locale does
>   **not** auto-flip it.
> - **Month switcher:** the "two parallel rows of pills" (DualMonthSwitcher) is
>   **DEFERRED** for PR4b — keep the existing chevron month nav.
> - **Activity dots:** use the **FIVE-source** model (task, habit, appointment,
>   medicine, prayer) with the prayer gate, per `DUAL_DATE_SPEC` — not the dots here.
> - **First-of-Hijri-month label:** use `HIJRI_MONTH_ABBREVIATIONS.md`, not an ad-hoc name.
> - **The PR4b audit is ALREADY DONE** (`design-context/_audit_calendar_dual.md`). Do
>   **not** re-run the "audit-first / gap report" workflow described below for PR4b.
> - **Section B (Focus oil) is PR8** — ignore for PR4b.
