...
- Mirror for RTL: pass `Directionality.of(context) == TextDirection.rtl`
  ...
  `Directionality`).
- On phone, the "+" FAB is a **standalone pill outside** the liquid-glass
  bar (right of the bar in LTR / left in RTL — see `REDESIGN_AUDIT.md §11`).
  On tablet+, that same FAB widget moves into the rail's `leading:` slot,
  kept centered above the destinations.
...
**Targets:** `lib/features/home/presentation/pages/onboarding_page.dart` (existing, unchanged for Variant A) plus the new variant pages (`onboarding_restyled_page.dart`, `onboarding_short_page.dart`, `onboarding_expanded_page.dart`) introduced by `PR-ONBOARD-AB`. See `ONBOARDING_AB_SPEC.md` and `REDESIGN_AUDIT.md §10`. Do **not** create `lib/features/onboarding/`.
