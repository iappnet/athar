...
| **PR-COMPONENT-P1** — `accentAmber` token (light `0xFFCA8A21` / dark `0xFFE8B84B`) added to `athar_colors.dart` all 6 sites; Calibri+fontFallback on 23 TextStyles (`athar_text_field` ×10, `time_slot_picker` ×10, `icon_picker` ×1, `athar_card` ×2); `AtharEmptyState` (`athar_display.dart`) title+message TextStyles get Calibri; 7 inline empty states → `AtharEmptyState` (medicines, appointments, vitals, health_timeline, notifications, space_page, space_list); `_getPeriodInfo` Colors.xxx→accent tokens; 686-line dead block stripped; P7/P8/P9 logged in KNOWN_PROBLEMS | `bb0c33d` | (in branch) | ✅ `flutter analyze` 0 issues · P8/P9 doc-noted; My Day + smart_habits_strip deferred to P2 | 2026-06-03 |
