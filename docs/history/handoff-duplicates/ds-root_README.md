...
- **Type**: Cairo (Arabic + Latin fallback) · Inter (English-only) · JetBrains Mono (numerals).
...
- **Fonts**: Cairo `.ttf` ships in the repo at `assets/fonts/`; this workspace falls back to Google Fonts for the previews. Production code should keep using the bundled fonts.
