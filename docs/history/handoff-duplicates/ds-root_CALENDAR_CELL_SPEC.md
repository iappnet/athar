...
| bottom-center, 3pt from bottom | Hijri numeral | `AppText.captionS` (10pt), Cairo, `AppColors.primary` |
...
- Hijri numeral position **replaced** by short Hijri month name (e.g. "رجب",
  truncated to 6 chars) for that single cell.

### Activity dots
- Row of 1–3 dots, 4×4pt, 2pt gap, centered.
- Order: task (`AppColors.taskBlue`), habit (`AppColors.success`), prayer (`AppColors.primary`).
...
- Secondary line: Hijri span — e.g. "رجب – شعبان ١٤٤٦" — `AppText.bodyS`,
  `AppColors.primary`, Cairo, `direction: rtl`.

When `primaryHijri = true`: swap. Hijri title on top (Cairo, `titleXL`),
...
- Row 2 — Hijri (RTL): 12 pills, Cairo, current = `primary` border + ink,
