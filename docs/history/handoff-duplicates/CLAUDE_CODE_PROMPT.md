...
- Read `PR3_DESIGN_RULINGS.md` first — it resolves all 6 PR3 blockers (countdown = **44px** not 64px; **JetBrains Mono** for numerals; **no adhanMoment/ALLAHU AKBAR state — Phase 7 cancelled**; nafl chip Option A; expanded only on Prayer page + iPad, no on-card toggle; iPhone SE accepted). No item is "awaiting designer."
- Rebuild per `PRAYER_CARD_SPEC.md` v2.1 (reconciled 2026-05-31). Read it end-to-end before writing code. Hard rules: **44pt countdown** (never 64pt), forest gradient `#0F3D2E → #1A5A45`, no continuous animation, no "ALLAHU AKBAR" moment, whole card → existing `PrayerDetailsPage` (no separate "Start dhikr" pill). **Preserve** the existing 40-min post-prayer dhikr window and the Duha / Qiyam nafl chip logic in `PrayerCubit` — restyle only.
- Use existing `UserSettings.prayerCardDisplayMode`. Compact/expanded variants are local widget state. Do **not** regress Phase 8.1 hierarchy (3 enforcement points: `smart_prayer_wrapper.dart:30`, `prayer_notification_scheduler.dart:35,209,271`, `prayer_conflict_service.dart:16`).
